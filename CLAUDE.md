...
- **Tests:** no external runner. The suite is *in-app* — open the app and click the **"Run algorithm tests"** link (calls `runTests()`, index.html:2354). It runs ~102 assertions in ten groups (core `evaluate()` cases, the Field View `MAX(rows)==combined` invariant, advisor raise/lower **reachability**, the record-caps-field **UI effect** cases, and two **framework parity** groups: case-insensitive role matching + the filtered-`*` defect, and which same-named block the framework uses; plus the held-back marker check across Matrix, Trace and Field View, and the child-list Add/Remove button rule; plus the Trace sheet's compact view keeping every cell the answer used) and prints `N / N passed` in the right pane. The link forces Trace mode so results are visible from any mode.
