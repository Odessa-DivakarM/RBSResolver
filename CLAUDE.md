...
- **Tests:** no external runner. The suite is *in-app* — open the app and click the **"Run algorithm tests"** link (calls `runTests()`, index.html:2354). It runs ~106 assertions in eleven groups (core `evaluate()` cases, the Field View `MAX(rows)==combined` invariant, advisor raise/lower **reachability**, the record-caps-field **UI effect** cases, and two **framework parity** groups: case-insensitive role matching + the filtered-`*` defect, and which same-named block the framework uses; plus the held-back marker check across Matrix, Trace and Field View, and the child-list Add/Remove button rule; plus the Trace sheet's compact view keeping every cell the answer used, and the Open-child / Back round trip — which drives the real UI on a synthetic parent pushed into `parsed.Entities` and restores all state it touches, so it needs a loaded workbook) and prints `N / N passed` in the right pane. The link forces Trace mode so results are visible from any mode.
...
- **Trace** (`'single'`, `renderResults` index.html:1846, narrative in `buildNarrativeTrace`) — a user's roles + one block/field → the full step-by-step cascade explanation. On a large block (>12 rows or >10 role columns) the sheet table shows only the selected row and this user's + `*` columns, with "show all" toggles (`sheetShowAll`, `sheetUsedColumns`); with >3 matched roles the narrative collapses to one line per role. A child-list Add/Remove row gets an "Also checked: the <child> record" panel (`buildChildPanel`, fed by `lim.child.trace` — the same evaluation the 🔒 uses) and an "Open in Trace" / "← Back" pair (`traceReturn`, cleared whenever the user navigates themselves). Once a result is shown, any input change on the left re-evaluates it (`refreshTrace`) — a Trace result must never describe inputs that are no longer selected.
